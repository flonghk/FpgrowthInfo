...
public class FpTree {
	...
	int s = cd.fPSupport();
	//public static final int  support = s; // 设定最小支持频次为2  
	public final int  support = s; // 设定最小支持频次为2 
    //保存第一次的次序  
    ...
    public LinkedList<LinkedList<String>> readTransRocords(String... filenames) {
    	LinkedList<LinkedList<String>> transaction = null;
        if (filenames.length > 0) {
            transaction = new LinkedList<LinkedList<String>>();
    ...
    public void fpgrowth(LinkedList<LinkedList<String>> records,String item){  
        ...
        if(item!=null){  
            ...
            for(int i=header.size()-1;i>=0;i--){  
                ...
                FpgrowthModel fpm = new FpgrowthModel();
                FpgrowthDao fpd = new FpgrowthDao();
                fpm.setfpCount(count);
                String _item = head.getName()+","+item;
                fpm.setfPName(_item);
                String[] str=_item.split(",");    
                fpm.setfpNameCount(str.length);
                fpd.insertFpgrowtDetail(fpm);
